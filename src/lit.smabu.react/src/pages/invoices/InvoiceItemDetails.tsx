    ...
    const handleSubmit = (event: React.FormEvent) => {
        ...
        axiosConfig.put<UpdateInvoiceItemCommand>(`invoices/${params.invoiceId}/items/${params.id}`, {
            ...
            .then(response => {
                setLoading(false);
                setError(null);
                toast("Rechnungsposition erfolgreich gespeichert: " + response.statusText, "success");
                loadData();
            })
            .catch(error => {
                setError(error);
                    ...
                    <DefaultContentContainer title={invoice?.displayName} subtitle={"#" + data?.displayName} loading={loading} error={error} toolbarItems={toolbarItems} >
                        <Paper sx={{ p: 2 }}>
                            <Grid container spacing={2}>
                                ...
                                <Grid size={{ xs: 6, sm: 6, md: 3 }}><TextField fullWidth label="Einheit" name="quantity.unit" value={data?.quantity?.unit} onChange={handleChange} required /></Grid>
